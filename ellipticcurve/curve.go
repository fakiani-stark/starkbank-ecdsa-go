...
//
func (obj CurveFp) Contains(p Point) bool {
	u := new(big.Int)
	v := u.Exp(p.Y, big.NewInt(2), nil)
	w := u.Exp(p.X, big.NewInt(3), nil)
	w = u.Add(w, obj.A)
	w = u.Add(w, u.Mul(obj.A, p.X))
	w = u.Add(w, obj.B)
	v = u.Sub(v, w)
	v = u.Mod(v, obj.P)
...
func CurveByOid(oid []int) CurveFp {
	var listCurveByOid CurveFp
	for _, curve := range SupportedCurves() {
		if len(curve.Oid) == len(oid) {
			listCurveByOid = curve
		}
	}
